...
def simulate_iteration(eig1, eig2, x_aug, n_time, rng, clip):
    n_sub, n_node = x_aug.shape[0], eig1.eigenvectors.shape[0]
    ...
    for i in range(n_sub):
        ...
        t = np.sqrt(x_aug[i, 0]) * (trand * eig1.sqrt_eigenvalues_pos).dot(eig1.eigenvectors.T) + \
            np.sqrt(x_aug[i, 1]) * (trand * eig1.sqrt_eigenvalues_neg).dot(eig1.eigenvectors.T)
        
        if eig2 is not None:
            trand2 = rng.standard_normal((n_time, n_node))
            t += np.sqrt(x_aug[i, 2]) * (trand2 * eig2.sqrt_eigenvalues_pos).dot(eig2.eigenvectors.T) + \
                 np.sqrt(x_aug[i, 3]) * (trand2 * eig2.sqrt_eigenvalues_neg).dot(eig2.eigenvectors.T)
 
        C, R = np.cov(t, rowvar=False), np.corrcoef(t, rowvar=False)
        Z = fisher_z_from_corr(R, clip=clip)
        s_cov += vectorize_uplo(C); s_cor += vectorize_uplo(R); s_z += vectorize_uplo(Z)
...
def main():
    ...
    ap.add_argument("NREP", type=int); ap.add_argument("UNUSED_ARG")
    ...
    avg_mat, selected_pconns = get_averaged_pconn(args.PCONNDIR, NUMTEMP, args.seed)
